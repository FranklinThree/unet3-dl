...
def get_argument():
    ...
    work_dir = "E:/Programming/python/pytorch/Unet3+"
    os.chdir(work_dir)
    # Data settings
    parser.add_argument('--tre_file_path',
                        default=work_dir + '/Datasets/Brain_MR_Angiography/TubeTK/original/Vessel_annotation_file')
    parser.add_argument('--nii_save_path', default=work_dir + '/Datasets/Brain_MR_Angiography/TubeTK/original/Vessel')
    parser.add_argument('--label_mha_save_path', default=work_dir + '/Datasets/Brain_MR_Angiography/TubeTK/original/Vessel_MHA')

    parser.add_argument('--org_MRA_path', default=work_dir + '/Datasets/Brain_MR_Angiography/TubeTK/original/MRA')
...
def main():
    ...
    if not os.path.exists(args.nii_save_path):
        os.makedirs(args.nii_save_path)
    if not os.path.exists(args.label_mha_save_path):
        os.makedirs(args.label_mha_save_path)
    ...
    for ann in ann_list:
                    ...
                    for k in range(int(np.ceil(2 * r))):
                        for j in range(int(np.ceil(2 * r))):
                            for i in range(int(np.ceil(2 * r))):
                                if np.sqrt(np.square(origin[0] + k - z) + np.square(origin[1] + j - y) + np.square(
                                        origin[2] + i - x)) <= r and origin[0] + k < 128:
                                    mask_arr[origin[0] + k, origin[1] + j, origin[2] + i] = 1

        # create NIfTI file for mask_arr
        org_img = sitk.ReadImage(fileName=args.org_MRA_path + '/' + ann[:-4-16] + '-MRA.mha')
        ...
        # sitk.WriteImage(image=mask_img,
        #                 fileName=args.nii_save_path + '/' + ann[:-4] + '.nii.gz')
        sitk.WriteImage(image=mask_img,
                        fileName=args.label_mha_save_path + '/' + ann[:-4] + '.mha')
