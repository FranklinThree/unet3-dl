...
class MRI_Loader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label_path = image_path.replace('image', 'label').replace('.tif', '_mask.tif')
        # 读取训练图片和标签图片
...
if __name__ == '__main__':
    log.init()
    train_dataset = MRI_Loader(yml_config["train"]["dataset_path"])

    test_dataset = MRI_Loader(yml_config["test"]["dataset_path"])
    # print(test_dataset.__len__())
    test_dataset.__getitem__(0)
    # 保存测试集和训练集到文件
    trds = open('train_dataset_o', 'wb')
    pickle.dump(train_dataset, trds, -1)
    logging.info("训练集数量",train_dataset.__len__())
    trds.close()

    tds = open('test_dataset_o', 'wb')
